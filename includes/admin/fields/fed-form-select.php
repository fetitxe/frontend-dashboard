...
function fed_form_select( $attributes ) {
	...
	$id       = ( '' != fed_get_data('id_name', $attributes) )? ( 'id="' . esc_attr( $attributes['id_name'] ) . '"' ) : null;
    ...
    foreach ($options as $key => $label) {
        if (is_array($value)) {
			...
			$option .= '<option value="' . esc_attr( $key ) . '" ' . selected($value, $key, false) . '>' . $label . '</option>';
		}
    }

    return sprintf(
        "<select name='%s' class='%s' %s %s %s %s %s>%s</select>",
        $select_name,
        $class,
        $multi_select,
        $disabled,
