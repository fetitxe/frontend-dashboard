			...
			'Assign Login Menu' => array(
				...
				'input'        => fed_get_input_details(
					array(
						...
						'input_value' => array('none' => __('Disable', 'frontend-dashboard')) + get_registered_nav_menus(),
				...
				'help_message' => fed_show_help_message(
					array(
						'content' => __('Select the respective menu items to show the Login, Logout and Dashboard Menu Items', 'frontend-dashboard'),
					)
				),
			),
		),
...
function fed_admin_frontend_login_menu_save( $request ) {
	...
	wp_send_json_success( 
		array( 
			'message' => __( 'Login menu successfully assigned', 'frontend-dashboard')
		)
	);
