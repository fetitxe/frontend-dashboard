	...
	class FEDInvoiceTemplate {
		...
		public function template() {
            ...
            if ($templates && count($templates) > 0) {
                ...
                ?>
                <form method="post" class="fed_ajax" action="<?php
					echo esc_url(
						add_query_arg(
							array( 
								'fed_action_hook' => 'FEDInvoiceTemplate@update'
							),
                            ...
                            <div class="col-md-4">
                                <div class="panel panel-secondary-heading">
                                    ...
                                    <div class="panel-body">
										<img alt="" class="img-responsive" src="<?php echo esc_url( $template['image'] ); ?>">
										<div class="fed_flex_center">
                                            <label>
												<input class="radio" type="radio" name="template" value="template-1" <?php echo isset( $settings['invoice']['template']['default'] ) && 'template-1' === $settings['invoice']['template']['default'] ? 'checked' : ''; ?>/>
											</label>
                                        </div>
                                        <div class="text-center">
                                            <strong><?php esc_attr_e('Select to make default', 'frontend-dashboard'); ?></strong>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <button class="btn btn-primary" type="submit"><?php esc_attr_e( 'Submit', 'frontend-dashboard' ); ?></button>
                </form>
                <?php
            } else {
