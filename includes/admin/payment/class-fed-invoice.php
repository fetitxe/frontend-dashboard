...
if ( ! class_exists('FEDInvoice')) {
	...
	class FEDInvoice {
        ...
        public function update($request)        {
            ...
            $new_settings = apply_filters('fed_payment_invoice_template_update', $invoice, $request);
        ...
        public function download($request){
			...
			if ( ! $settings ) {
				wp_send_json_error(
					array(
						'message' => __('Please configure the Invoice Details ( FED | Payments | Invoice | Company Details )', 'frontend-dashboard'),
					)
				);
			}
            global $wpdb;
            ...
            $width        = isset($settings['details']['width']) ? sprintf('width=%s', $settings['details']['width']) : '';
            $height       = isset($settings['details']['height']) ? sprintf('height=%s', $settings['details']['height']) : '';
            ...
            foreach ($payment['payment_items'] as $items) {
                ...
                $discount      = isset($item['discount_value']) && ! empty($item['discount_value']) ? sprintf('Discount : %s %s', esc_attr($item['discount_value']), fed_get_discount_type(esc_attr($item['discount']))) : '';
                $tax           = isset($item['tax_value']) && ! empty($item['tax_value']) ? sprintf('Tax : %s %s', esc_attr($item['tax_value']), fed_get_discount_type(esc_attr($item['tax']))) : '';
		...
		public function user() {
            ...
            $profiles = array(
            	'' => __('Hide this field')
            ) + (array) fed_convert_array_object_to_key_value($user_profiles, 'id', 'label_name');
            $settings = get_option('fed_invoice_settings');
		...
		public function store_user( $request ) {
            ...
            $new_settings = apply_filters('fed_invoice_user_address_data_update', $invoice, $request);
