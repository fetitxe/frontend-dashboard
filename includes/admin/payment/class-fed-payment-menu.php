            ...
            if ($menu) {
                if (isset($menus[$menu]['submenu']) && is_array($menus[$menu]['submenu']) && count($menus[$menu]['submenu'])) {
                        ...
                        <div class="col-md-3">
                            <div class="list-group">
                                <?php foreach ($menus[$menu]['submenu'] as $index => $sub_menu) {
                                    ...
                                    ?>
									<a class="list-group-item <?php echo esc_attr( $active ); ?>" href="<?php
										echo esc_url(
											fed_menu_page_url(
												'fed_payments', array(
													'menu'    => $menu,
										...
										?>">
                                        <span class="<?php echo esc_attr( $sub_menu['icon'] ); ?>"></span>
                            ...
                            <div class="panel panel-primary">
                                <div class="panel-heading">
                                    <h3 class="panel-title">
                                        <span class="<?php echo esc_attr($menus[$menu]['submenu'][$submenu]['icon']); ?>"></span>
                    ...
                    if (isset($menus[$menu]['submenu']) && is_string($menus[$menu]['submenu'])) {
						...
						if ( isset( $menus[ $menu ]['routes'] ) && is_array( $menus[ $menu ]['routes'] ) ) {
							...
							if ( array_key_exists( $route_value, $menus[ $menu ]['routes'] ) ) {
								...
								$extra_label     = ' - ' .fed_get_data($menu . '.routes.' . $route_value . '.name', $menus );
							}
						}
                        ?>
                        <div class="row">
                    ...
                    elseif (isset($menus[$menu])) {
						...
						<div class="alert alert-danger">
							...
							<strong><?php esc_attr_e('Invalid Menu Item | Error FED|Admin|Payment|FEDPaymentMenu@body_content', 'frontend-dashboard'); ?></strong>
						</div>
						<?php
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-danger">
                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                            <strong><?php esc_attr_e('Ah! something missing | Error FED|Admin|Payment|FEDPaymentMenu@body_content','frontend-dashboard'); ?></strong>
                        </div>
						<?php
						/** FED_Log::writeLog(
