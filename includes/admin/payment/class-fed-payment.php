	...
	class FEDPayment {
				...
				'input' => array(
					'Enable Payment' => array(
						...
						'help_message' => fed_show_help_message(
							array(
								'content' => __('By Checking this, you are enabling the Payment', 'frontend-dashboard')
							)
						),
					),
				),
		...
		public function update( $request ) {
			...
			wp_send_json_success(
				array( 
					'message' => __('Payment Settings Successfully Saved', 'frontend-dashboard' )
				)
			);
		}
		...
		public function authorize() {
            if ( ! fed_is_admin()) {
				wp_die(
					__('Sorry! You are not allowed to do this action | Error: FED|Admin|Payment|FEDPayment@authorize', 'frontend-dashboard')
				);
            }
