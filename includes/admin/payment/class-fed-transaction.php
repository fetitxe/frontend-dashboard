...
if ( ! class_exists('FEDTransaction')) {
    ...
    class FEDTransaction{
        ...
        public function update($request){
            ...
            $validate->name(__('Transaction ID', 'frontend-dashboard'))->value(fed_get_data('transaction_id'))->required();
            $validate->name(__('Purchase Date', 'frontend-dashboard'))->value(fed_get_data('created'))->required();
            $validate->name(__('Payment Source', 'frontend-dashboard'))->value(fed_get_data('payment_source'))->required();
            $validate->name(__('Payment Type', 'frontend-dashboard'))->value(fed_get_data('fed_pp_object_type'))->required();
            $validate->name(__('Product Name', 'frontend-dashboard'))->value(fed_get_data('fed_pp_object_id'))->is_array(1);
		...
		public function format_transaction( $request, $payment_type = '' ) {
            ...
            $transaction = array(
                ...
                'created'        => isset($request['created'])? date('Y-m-d H:i:s', strtotime(fed_sanitize_text_field($request['created']))) : '',
        ...
        public function add_items($request){
            if (isset($request['type'])) {
                ...
                if ($wpdb->get_var("SHOW TABLES LIKE '{$table_name}' ") === $table_name) {
					...
					wp_send_json_error(
						array(
							'message' => 'Table '.$table_name.' exist but not records'
						)
					);
                }
                wp_send_json_error(
                	array(
                		'message' => __('Table not exist: ', 'frontend-dashboard') . $table_name
                	)
                );
            }
        }
