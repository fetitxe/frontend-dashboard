...
function fed_admin_setting_form_dashboard_menu_function(){
    ...
    if ('save' === $action) {
        if( empty($post['fed_menu_name'] )
            ...
            wp_send_json_error(
            	array(
            		'message' => __('Please fill required fields', 'frontend-dashboard')
            	)
            );
            exit();
        }
    ...
    if ('delete' === $action) {
        if ('' == $post_id) {
            wp_send_json_error(
            	array(
            		'message' => __( 'Sorry! please reload the page and try again', 'frontend-dashboard' )
            	)
            );
            exit();
        }
		...
		if ( isset( $check_menu[ 'user_' . $is_id['id'] ] ) && isset( $check_menu[ 'user_' . $is_id['id'] ]['submenu'] ) ) {
			wp_send_json_error(
				array(
					'message' => __('You are trying to delete a menu, which has Sub Menu(s), Please delete or move it to different Menu', 'frontend-dashboard')
				)
			);
		}
		...
		if ( 'success' === $is_delete ) {
			wp_send_json_success(
				array(
					'message' => $post['fed_menu_name'] . __(' has been successfully deleted', 'frontend-dashboard'),
		...
		wp_send_json_success(
			array(
				'message' => __('Something went wrong, please try again or report us', 'frontend-dashboard')
			)
		);
    }
...
function fed_admin_setting_upl_form_function(){
    ...
    if ( ! wp_verify_nonce($_REQUEST['fed_admin_setting_upl_nonce'], 'fed_admin_setting_upl_nonce')) {
		wp_send_json_error(
        	array(
        		'message' => __('Invalid Request', 'frontend-dashboard')
        	)
		);
        exit();
    }
...
function fed_user_profile_delete_function(){
    ...
    if ('delete' === $action) {
        ...
        if ($check_up_id instanceof WP_Error) {
			wp_send_json_error(
				array(
					'message' => $check_up_id->get_error_message('fed_no_row_found_on_that_id') 
				)
			);
			exit();
        }
        ...
        if ('success' === $is_delete) {
			wp_send_json_success(
				array(
					'message' => $post['profile_name'] . __(' has been successfully deleted', 'frontend-dashboard'),
					'reload'  => $reload,
				)
			);
			exit();
