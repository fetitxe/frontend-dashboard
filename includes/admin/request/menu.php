...
function fed_admin_menu_save($request, $post_id = ''){
    ...
    if ('' != $post_id) {
		...
		if ( false === $status ) {
			wp_send_json_error( 
				array( 
					'message' => 'Sorry no record found to update your new details' 
				)
			);
		}
		...
		if ( false === $status ) {
			wp_send_json_error(
				array( 
					'message' => 'Sorry, Something went wrong in storing values in DB, please try again later or contact support'
				)
			);
		}
...
function fed_admin_menu_restrictive_words_check( $slug ) {
    ...
    if (in_array($slug, $restrictive_menu, false)) {
       wp_send_json_error(
        	array(
        		'message' => 'Sorry, You cannot use the restrictive slug name "'.$slug.'"'
        	)
       );
    }
}
    ...
    if (isset($request['data'])) {
        foreach ($request['data'] as $data) {
			...
			if (
                ...
                wp_send_json_error(
                	array(
                		'message' => 'There is some issue in your custom menu, please check'
                	)
                );
            }
        }
