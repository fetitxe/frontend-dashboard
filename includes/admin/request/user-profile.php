...
function fed_save_profile_post($request, $action = '', $post_id = ''){
	...
	if( 'profile' === $action ){
		...
		wp_send_json_error(array(
			'message' => __('Hey, you are trying something naughty', 'frontend-dashboard')
		));
	}
	...
	if( !empty($post_id) ){
		...
		if( null !== $duplicate ){
			wp_send_json_error(array(
				...
				/* Translators: 1%s 2%s : 1. Label Name  2. Input Type*/
					...
					esc_attr(strtoupper($duplicate->label_name)),
					strtoupper(fed_convert_this_to_that($duplicate->input_type,'_', ' '))
				),
			));
		}
		...
		if( null !== $duplicate ){
			...
			wp_send_json_error(array(
				...
				/* Translators: %1$s : Label Name, %2$s : Error Message 1, %3$s : Error Message 2  */
					__('Sorry, you have previously added %1$s  with input type %2$s on %3$s', 'frontend-dashboard'),
					esc_attr(strtoupper($duplicate->label_name)),
		 ...
		 */
		$status = $wpdb->insert($table_name, $request);

		if( false === $status ){
			wp_send_json_error(array(
				'message' => __('Sorry, Something went wrong in storing values in DB, please try again later or contact support','frontend-dashboard'),
			));
		}
